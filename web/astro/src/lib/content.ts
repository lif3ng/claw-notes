...
function getContentDir(): string {
  const candidates = [
    ...
    '/root/.openclaw/workspace/claw-notes/content',
  ];
...
export function getPost(year: string, slug: string): Post | null {
  ...
  const contentZh = fs.existsSync(zhPath) ? fs.readFileSync(zhPath, 'utf-8') : '';
